...
## ----cache = FALSE---------------------------------------------------------------------------------------------------------
# test unweighted mean function using Millsap & Kwok (2004) Example
PartInv(.25, kappa_r = 0.5, kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))

# obtain the selection accuracy of the strict invariant model
PartInv(.25, kappa_r = 0.5, kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))
...
## ----cache = FALSE---------------------------------------------------------------------------------------------------------
## @knitr PartInv_we function
## Partial Invariance Function for unweighted means 
...
## ----cache = FALSE---------------------------------------------------------------------------------------------------------
## @knitr example
# Test partial invariance function for unweighted mean for strict invariance (baseline)
strict_inv_un_base <- PartInv(.25, kappa_r = 0.5, kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))


## ----cache = FALSE---------------------------------------------------------------------------------------------------------
## @knitr strict invariance
# Test partial invariance function for weighted mean for strict invariance
# same as unweight mean
strict_inv_we_alleq <- PartInv_we(.25, kappa_r = 0.5, c = c(1,1,1,1), kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))
# excluding item 1
strict_inv_we_e1 <- PartInv_we(.25, kappa_r = 0.5, c = c(0,1,1,1), kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))
# excluding item 2
strict_inv_we_e2 <- PartInv_we(.25, kappa_r = 0.5, c = c(1,0,1,1), kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))
# excluding item 3
strict_inv_we_e3 <- PartInv_we(.25, kappa_r = 0.5, c = c(1,1,0,1), kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))
# excluding item 4
strict_inv_we_e4 <- PartInv_we(.25, kappa_r = 0.5, c = c(1,1,1,0), kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))
# different weight combinations
strict_inv_we_allun <-PartInv_we(.5, kappa_r = 0.5, c = c(1,0.7,0.8,1), kappa_f = 0, phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), tau_r = c(.225, .025, .010, .240),
        Theta_r = diag(.96, 4))
...
## ----cache = FALSE---------------------------------------------------------------------------------------------------------
## @knitr partial invarance
# test partial invariance function for weighted mean for partial strict invariance
# include all items
par_stri_we_alleq <- PartInv_we(propsel = .25, kappa_r = 0.5, kappa_f = 0, c = c(1,1,1,1), phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), lambda_f = c(.3, .5, .7, .5),
        tau_r = c(.225, .025, .010, .240), tau_f = c(.225, -0.225, .240, -.025),
        Theta_r = diag(.96, 4))
# exclude item1
par_stri_we_ex1 <- PartInv_we(propsel = .25, kappa_r = 0.5, kappa_f = 0, c = c(0,1,1,1), phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), lambda_f = c(.3, .5, .7, .5),
        tau_r = c(.225, .025, .010, .240), tau_f = c(.225, -0.225, .240, -.025),
        Theta_r = diag(.96, 4))
# exclude item2
par_stri_we_ex2 <- PartInv_we(propsel = .25, kappa_r = 0.5, kappa_f = 0, c = c(1,0,1,1), phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), lambda_f = c(.3, .5, .7, .5),
        tau_r = c(.225, .025, .010, .240), tau_f = c(.225, -0.225, .240, -.025),
        Theta_r = diag(.96, 4))
# exclude item3
par_stri_we_ex3 <- PartInv_we(propsel = .25, kappa_r = 0.5, kappa_f = 0, c = c(1,1,0,1), phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), lambda_f = c(.3, .5, .7, .5),
        tau_r = c(.225, .025, .010, .240), tau_f = c(.225, -0.225, .240, -.025),
        Theta_r = diag(.96, 4))
# exclude item4
par_stri_we_ex4 <- PartInv_we(propsel = .25, kappa_r = 0.5, kappa_f = 0, c = c(1,1,1,0), phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), lambda_f = c(.3, .5, .7, .5),
        tau_r = c(.225, .025, .010, .240), tau_f = c(.225, -0.225, .240, -.025),
        Theta_r = diag(.96, 4))
# different weight combinations
par_stri_we_allun <- PartInv_we(propsel = .25, kappa_r = 0.5, kappa_f = 0, c = c(1,0.7,0.8,1), phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), lambda_f = c(.3, .5, .7, .5),
        tau_r = c(.225, .025, .010, .240), tau_f = c(.225, -0.225, .240, -.025),
        Theta_r = diag(.96, 4))
# less weight to noninvariant items
par_stri_we_allunew <- PartInv_we(propsel = .25, kappa_r = 0.5, kappa_f = 0, c = c(1,0.7,0.3,0.8), phi_r = 1,
        lambda_r = c(.3, .5, .9, .7), lambda_f = c(.3, .5, .7, .5),
        tau_r = c(.225, .025, .010, .240), tau_f = c(.225, -0.225, .240, -.025),
        Theta_r = diag(.96, 4))
...
## --------------------------------------------------------------------------------------------------------------------------
## @knitr displayresults
rbind(strict_inv_un_base, strict_inv_we_alleq,strict_inv_we_e1,strict_inv_we_e2, strict_inv_we_e3,strict_inv_we_e4,strict_inv_we_allun)
rbind(par_stri_we_alleq,par_stri_we_ex1, par_stri_we_ex2,par_stri_we_ex3,par_stri_we_ex4,par_stri_we_allun, par_stri_we_allunew)
strict_results <- c(strict_inv_un_base[4], strict_inv_we_alleq[4],strict_inv_we_e1[4],strict_inv_we_e2[4], strict_inv_we_e3[4],strict_inv_we_e4[4],strict_inv_we_allun[4])%>%
  knitr::kable("html", caption = "Strict Invariance")%>%
   add_header_above(c("unweighted all", "weight equal all", "exclude i1", "exclude i2","exclude i3","exclude i4", "unequal weight all"))%>%
  kable_styling(bootstrap_options = c("hover", "condensed"))
strict_results
partial_results <- c(par_stri_we_alleq[4],par_stri_we_ex1[4], par_stri_we_ex2[4],par_stri_we_ex3[4],par_stri_we_ex4[4],par_stri_we_allun[4], par_stri_we_allunew[4])%>%
    knitr::kable(caption = "Partial Strict Invariance")
    add_header_above(partial_results, c("weight equal all", "exclude i1", "exclude i2","exclude i3 ","exclude i4", "unequal weight all","new unequal weight all "))%>%
       kable_styling(bootstrap_options = c("hover", "condensed"))
